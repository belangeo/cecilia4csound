...
class Line:
    ...
    def getLineState(self):
        dict = {'data': self.normalize(),
                ...
                'bezierLeft': [bez for bez in self.bezierLeft],
                'bezierRight': [bez for bez in self.bezierRight]}
    ...
    def setLineState(self, dict):
        ...
        self.bezierLeft = dict.get('bezierLeft', [])
        self.bezierRight = dict.get('bezierRight', [])
        self.checkIfCurved()
    ...
    def getCsoundPoints(self):
        return self.csoundPoints

    def getTypes(self):
        types = []
        for i in range(len(self.typeLeft)):
            types.append(self.typeLeft[i])
            types.append(self.typeRight[i])
        return types

    def normalize(self):
        data = [p for p in self.getData()]
        yrange = self.getYrange()
        totaltime = CeciliaLib.getTotalTime()
        templist = []
        if self.getLog():
            for l in data:
    ...
    def setCurvedLine(self):
        ...
        self.bezierLeft = [5] * (self.getLength() - 1)
        self.bezierRight = [5] * (self.getLength() - 1)

        for k in range(len(data)-1):
            alpha1 = self.bezierLeft[k]
            alpha2 = self.bezierRight[k]
            x1 = data[k][0]
            x2 = data[k+1][0]
            y1 = data[k][1]
            y2 = data[k+1][1]
            midPointFrac = abs((float(alpha2) / alpha1) * .5)
            xc = x1 + ((x2-x1) * midPointFrac)
            if y1 < y2: midPointFrac = midPointFrac
            else: midPointFrac = 1 - midPointFrac
            yc = y1 + ((y2-y1) * midPointFrac)
            diff1 = yc - y1
            diff2 = y2 - yc
            length1 = xc - x1
            length2 = x2 - xc
            steps1 = int(num * length1)
            if steps1 > 1:
                inc1 = length1 / (steps1-1)
            steps2 = int(num * length2)
            if steps2 > 1:
                inc2 = length2 / (steps2-1)

            self.csoundPoints.append([x1,y1])
            self.csoundPoints.append([xc,yc])
            if y1 < y2:
                if y1 < yc: type1 = alpha1
                else: type1 = -alpha1
                if yc < y2: type2 = -alpha2
                else: type2 = alpha2
            else:
                if y1 < yc: type1 = -alpha1
                else: type1 = alpha1
            ...
            self.typeLeft.append(type1)
            self.typeRight.append(type2)
            ...
            if steps1 <= 1:
                ...
                for i in range(steps1):
                    t = float(i) / (steps1-1)
                    x = x1 + inc1 * i
                    val = y1 + diff1 * (1 - math.exp(type1*t)) / (1 - math.exp(type1))
                    self.lines.append([x, val])

            if steps2 <= 1:
                self.lines.append([xc, yc])
            elif alpha2 == 0:
                ...
                for i in range(steps2):
                    t = float(i) / (steps2-1)
                    x = xc + inc2 * i
                    val = yc + diff2 * (1 - math.exp(type2*t)) / (1 - math.exp(type2))
                    self.lines.append([x, val])

        self.csoundPoints.append([x2,y2])
        self.csoundPoints = self.denormalize(self.csoundPoints)
...
class Grapher(plot.PlotCanvas):
    def __init__(self, parent, style=wx.EXPAND):
        ...
        self.parent = parent
        self.menubarUndo = self.parent.parent.menubar.FindItemById(wx.ID_UNDO)
        self.menubarRedo = self.parent.parent.menubar.FindItemById(wx.ID_REDO)
        self._history = []
    ...
    def OnMotion(self,event):
        ...
        if self._tool == 0:
            ...
            if self.point != None:
                if not self.selectedPoints:
                    ...
                    for p in selectedPoints:
                        ...
                        # TIP: holding Alt key while moving a point clip the X axis 
                        # TIP: holding Shift and Alt keys while moving a point clip the Y axis 
                        if self.data[self.selected].getLog():
            ...
            elif pos[0] > CeciliaLib.getTotalTime(): pos = [CeciliaLib.getTotalTime(), pos[1]]
            ...
            if distance > 0.001:
                ...
                if pos[0] >= points[-1]:
                    line.deletePoint(-1)
                    line.data.append([CeciliaLib.getTotalTime(), pos[1]])
                    self._pencilData.append(pos)
                elif pos[0] <= points[0]:
                    line.deletePoint(0)
    ...
    def OnKeyDown(self, event):
        ...
        if key == 118: # v
        ...
        elif key == 122 and not event.CmdDown(): # z
            ...
            if self.selectedPoints:
                ...
                for p in points:
                    if not p[0] in [0.0, CeciliaLib.getTotalTime()]:
...
class ToolBar(wx.Panel):
    def __init__(self, parent, size=(-1,25), tools=[], toolFunctions=None):
        ...
        if CeciliaLib.getModuleDescription() != '':
            helpButton = CloseBox(fakePanel, size=(18,18), pos=(25,2), outFunction=self.onShowModuleDescription, label=CeciliaLib.getCurrentCeciliaFile())
        ...
        self.box.Add(self.menu, 0, wx.BOTTOM | wx.LEFT | wx.TOP, 5)
        self.box.Add(self.toolbox, 0, wx.BOTTOM | wx.LEFT | wx.TOP, 5)
        self.box.Add(self.convertSlider, 0, wx.ALIGN_CENTER_VERTICAL | wx.LEFT, 6)
        self.box.Add(fakePanel, 0, wx.EXPAND | wx.RIGHT, 20)
        self.box.Add(self.radiotoolbox, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 20)
        self.box.Add(self.palettetoolbox, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 20)
    ...
    def onShowModuleDescription(self):
        ...
        pop = TextPopupFrame(self, CeciliaLib.getModuleDescription(), pos=(pos[0]-100,pos[1]+20))
...
class CursorPanel(wx.Panel):
    ...
    def OnPaint(self, evt):
        ...
        gc.SetPen(wx.Pen(BACKGROUND_COLOUR))
        gc.SetBrush(wx.Brush(BACKGROUND_COLOUR))
        tri = [(self.time, h-1), (self.time-4, h-7), (self.time+4, h-7), (self.time, h-1)]
        gc.DrawLines(tri)
...
class CECGrapher(wx.Panel):
    def __init__(self, parent, id=wx.ID_ANY, pos=(20,20), size=(100, 100)):
        ...
        mainBox = wx.FlexGridSizer(3, 1, 0, 0)
    ...
    def checkForAutomation(self):
        ...
        sl = None
        if CeciliaLib.getSamplerSliders():
            for slider in CeciliaLib.getSamplerSliders():
                if slider.getPath() and slider.getRec():
                    ...
                    path = slider.getPath()
                    data = convert(path, slider, thresholdX, thresholdY)
                    for line in self.plotter.getData():
                        if line.getName() == slider.getCName():
                            ...
                            slider.setRec(0)
                            slider.setPlay(1)
        if CeciliaLib.getUserSliders():
            for slider in CeciliaLib.getUserSliders():
                if slider.getPath() and slider.getRec():
                    sl = slider
                    slider.setAutomationLength(CeciliaLib.getControlPanel().getTime())
                    path = slider.getPath()
                    if type(path) not in [ListType, TupleType]:
                        data = convert(path, slider, thresholdX, thresholdY, which=None)
                        for line in self.plotter.getData():
                        ...
                        for i, p in enumerate(path):
                            ends = ['min', 'max']
                            data = convert(p, slider, thresholdX, thresholdY, which=i)
                            for line in self.plotter.getData():
            ...
            if type(path) in [ListType, TupleType]:
                for i in range(2):
                    if line.getLabel().endswith(ends[i]):
                        path = path[i]
                        break
                data = convert(path, slider, self.threshX, self.threshY, True, which=i)
            else:
                data = convert(path, slider, self.threshX, self.threshY, True)
            self.cecGrapher.setLineData(line, data)

    def onSlider1(self, value):
        val = value * .001
        self.threshX = self.thresholdX * val
        self.threshY = self.thresholdY * val
        self.rescale()

def checkFunctionValidity(func, totaltime):
    func = [float(v.replace('"', '')) for v in func.split()]
    func = [[func[i*2] * totaltime, func[i*2+1]] for i in range(len(func) / 2)]
    if func[0][0] != 0: func[0][0] = 0
    if func[-1][0] != totaltime: func[-1][0] = totaltime
    oldX = -1
    for f in func:
        if f[0] < oldX:
...
def buildGrapher(grapher, list, totaltime):
    ...
    for widget in list:
        if widget['type'] == 'cslider' and widget.get('rate', 'k') == 'k' and eval(widget.get('up', '0')) == 0:
            widgetlist2.append(widget)
            labelList.append(widget.get('label', ''))

    for widget in list:
        if widget['type'] == 'crange' and widget.get('rate', 'k') == 'k' and eval(widget.get('up', '0')) == 0:
            widgetlist2range.append(widget)
            labelList.append(widget.get('label', '') + ' min')
            labelList.append(widget.get('label', '') + ' max')

    for widget in CeciliaLib.getSamplerSliders():
        widgetlist3.append(widget)
        labelList.append(widget.getLabel())

    for plugin in CeciliaLib.getPlugins():
        if plugin != None:
    ...
    for i, widget in enumerate(widgetlist):
        ...
        maxi = eval(widget.get('max', '100'))
        init = eval(widget.get('init', str(mini)))
        unit = widget.get('unit', '')
        func = widget.get('func', '0 %f 1 %f' % (init, init))
        ...
        if name.startswith('-'):
            CeciliaLib.showErrorDialog('Error when building interface!', "Missing name. First argument of cgraph can't be %s." % widget['name'])
    ...
    for i, widget in enumerate(widgetlist2):
        ...
        for slider in CeciliaLib.getUserSliders():
            if slider.getName() == name:
                col3 = wx.Colour(colour[3][0], colour[3][1], colour[3][2], 128)
                slider.setFillColour(colour[1], colour[2], col3)
                sl = slider
                gen = slider.getTable()
        ...
        linelist.append([func, (mini, maxi), colour, label, log, name, gen, size, sl, ''])
    if linelist:
    ...
    for i, widget in enumerate(widgetlist2range):
        for j in range(2):
            ...
            maxi = eval(widget.get('max', '100'))
            init = eval(widget.get('init', str(mini)))
            func = widget.get('func', '0 %f 1 %f' % (init[j], init[j]))
            func = checkFunctionValidity(func, totaltime)
            ...
            linelist.append([func, (mini, maxi), colour, label, log, name, gen, size, sl, ends[j]])
    if linelist:
        grapher.createSliderLines(linelist)
...
def convert(path, slider, thresholdX, thresholdY, fromSlider=False, which=None):
    if not fromSlider:
        f = open(path, 'r')
        data = f.read().split('\n')
    ...
    # filter out points with too small deviation
    oldpente = 0
    for i in range(len(temp)-2):
        p1 = temp[i+1]
        p2 = temp[i+2]
        pente = ((p2[1] - p1[1]) / maxval) / (p2[0] - p1[0])
        if oldpente == 0:
            if pente != 0:
                if p1[0] - points[-1][0] > thresholdX:
                    points.append(p1)
                    points.append(p2)
        elif pente + oldpente < -thresholdY or pente + oldpente > thresholdY:
                if p1[0] - points[-1][0] > thresholdX:
                    points.append(p1)
